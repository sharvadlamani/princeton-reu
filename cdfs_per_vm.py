...
def normalize_sources(df):
    df['Source Group'] = df['Source'].apply(
        ...
        else x.lower().strip() if isinstance(x, str) and x.lower().strip() in [
            'tranco-top1k', 'tranco-rank10k', 'tranco-rank100k']
        else 'other'
    )
    return df
...
def compute_global_limits(std_files, prem_files, columns, source_groups):
    ...
    min_val = min(xlimits[col][0] for col in rtt_cols)
    for col in rtt_cols:
        xlimits[col] = [min_val, 1000]
    return xlimits
    ...
    for i, source in enumerate(source_groups):
        for j, column in enumerate(columns):
            ...
            for df, label in [(standard_df, "Standard"), (premium_df, "Premium")]:
                ...
                elif column == "RTT from Google Exit to Last Node (ms)":
                    values = values[(values >= 0) & (values <= 1000)]
                elif column == "RTT Ratio (Google/Total)":
...
def main():
    ...
    output_dir = os.path.expanduser("~/reu/figures/cdfs_vm")

    columns = [
        "Median RTT to Last Google Node (ms)",
        "Median RTT to Last Node (ms)",
        "RTT from Google Exit to Last Node (ms)",
        "RTT Ratio (Google/Total)",
